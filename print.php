...
function printNewReport($conn, $componentArray, $planArray, $planID) {
	$arrayDate = getdate();
	$root = 
	$fileName = ( '../script/' . $planArray['StrataNumber'] . '.$root' . $planArray['PlanId'] . '.' . $arrayDate['year'] . '-' . $arrayDate['mon'] . '-' . $arrayDate['mday'] . '.docx');
	...
	$picArray = getPictureArray($conn, $planID, $x);
	...
	$tCell->addPreserveText('Address: ' . $planArray['Street'] . ', ' . $planArray['City'] . ', BC, ' . $planArray['PostalCode'] . '					Component {PAGE} of {NUMPAGES}', $smallStyle, 'myParaStyle'); //Address of property here
	...
	$tCell->addText('STRATA RESERVE PLANNING - a division of Bramwell & Associates Realty Advisors Inc.                 Tel: 604-608-6161', $smallStyle, 'myParaStyle'); //unlikely to be changed. 
